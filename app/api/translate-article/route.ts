...
export async function POST(req: NextRequest) {
  ...
  const contentPrompt = [
    'Переведи следующий текст с английского языка на русский язык.',
    '',
    'КРИТИЧЕСКИ ВАЖНО:',
    '- Переведи ВЕСЬ текст полностью на русский язык',
    '- НЕ оставляй английские слова, если есть русский эквивалент',
    '- НЕ смешивай языки - используй ТОЛЬКО русский язык',
    '- НЕ добавляй иероглифы, арабские символы или другие языки',
    '- Сохрани оригинальную структуру, Markdown разметку и форматирование',
    '- Сохрани все заголовки, списки, ссылки и код-блоки',
    '- НЕ добавляй никаких пояснений, введения или комментариев',
  ...
  try {
    const response = await fetch(OPENROUTER_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'Ты — профессиональный переводчик с английского на русский язык. Твоя задача — создавать точные и естественные переводы. ВСЕГДА переводи полностью на русский язык. НИКОГДА не смешивай языки. НИКОГДА не добавляй иероглифы или символы других языков. Используй ТОЛЬКО русский язык и латиницу для технических терминов.' },
